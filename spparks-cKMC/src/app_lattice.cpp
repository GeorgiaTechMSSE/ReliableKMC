 ...
 *     - By Yan Wang (June  2010)
 ******************************  
       set[i].propensity_intval[m] = site_propensity_intval(set[i].site2i[m]);		  
/*****************************/ 	    
	  }
 ...
 *     - By Yan Wang (June  2010)
 ******************************  
      set[i].solve->init_intval(set[i].nlocal,set[i].propensity_intval);
...
//printf("setup():line364:sectorflag=%d\n",sectorflag);  
  ...
  // setup future output
//printf("setup():line448:before output->setup\n");  

  nextoutput = output->setup(time);
//printf("setup():line451:after output->setup\n");  
}
...
void AppLattice::iterate()
{
  ...
  if (solve) {
//printf("iterate():line458:sectorflag=%d\n",sectorflag);  
 ...
 ******************************  
  nlocal = set[0].nlocal;
  propensity_intval = set[0].propensity_intval;
  isiteList = set[0].isiteList;
/*****************************/ 	
  i2site = set[0].i2site;
...
//printf("iterate_kmc_global():line 496 \n");
...
/*****************************
	IntervalItem pdt;
    isite = solve->event_intval(isiteList, &pdt); //isite now is the integer number of sites (size of the random set) 
												 //isiteList contains a list of sites that have events
	dt = pdt.inf;											 
    timer->stamp(TIME_SOLVE);
    if (isite <= 0) done = 1;
    else {
...
//printf("\n isiteList[%d]=%d i2site[%d]=%d ",i,isiteList[i],isiteList[i],i2site[isiteList[i]]);
...
//			   propensity_intval[m].sup,propensity_intval[m].wid);
			...
			if(isiteList[i] != set[0].nlocal) // make sure it is not a null event - the last event is a null event		
			{
				site_event(i2site[isiteList[i]],ranapp);
				naccept++;
			}
		}
      time += dt;
 ...
 *     - By Yan Wang (June  2010)
 ****************************** 
		nlocal = set[iset].nlocal;
		propensity_intval = set[iset].propensity_intval;
/*****************************/ 	
 ...
 ******************************  
  ...
  memory->destroy_1d_T_array(set[iset].propensity_intval,0);
  memory->create_1d_T_array(set[iset].propensity_intval, 0, set[iset].nlocal-1, 
					     "appLattice:propensity_intval");
printf("set[%d].nlocal=%d\n",iset,set[iset].nlocal);					 
//	memory->sfree(set[iset].isiteList);
	// the maximum number of events to fire at one time = the total number of sites + one null event
	set[iset].isiteList = (int*) memory->smalloc( (set[iset].nlocal+1)*sizeof(int),
						"appLattice:isiteList");
