...
enum{ONE=-1,TWO=-2,THREE=-3,FOUR=-4,EVENTS=-10,CTRL=-20};
 ...
 ******************************/  
    ...
    for(int i=0;i<apperbium->numspecies;i++){
//printf("sitesp[%d]=%d ",i,sitesp[i]); 
		apperbium->pcount[i]=0;
...
/****commented by Yan Wang  and replaced by the following ********
////    for (int i = 0; i < nlocal; i++) sitesp[element[i]]++;
/**************************************************************/
    for (int i = 0; i < nlocal; i++) apperbium->pcount[element[i]]++;
